...
{
    int i,cpt=0;
    S_joueur jr_vainqueur;//on crée un S_joueur
    ...
    else if (jr_vainqueur.init_fin_manche==-1)//si il a inititié la fin de partie mais qu'il n'est pas premier au score de la manche
    {
        ...
        cpt++;
        jr[jr_vainqueur.joueur_no-1].score_manche=(jr[jr_vainqueur.joueur_no-1].score_manche)*2;//double le score de la manche du joueur ayant initié
    }
    cpt+=2;
    Positionner_Curseur(x,y+cpt);
...
void recap_fin_partie(S_joueur jr[],int nb_jr,int x,int y)
{
    ...
    afficher_boite_dialogue();
    printf("%s é dépassé les 100 pts, la partie est teminé!", jr[no_jr_de_fin-1].prenom);
    cpt+=2;
    Positionner_Curseur(x,y+cpt);
